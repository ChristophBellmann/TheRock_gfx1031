...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="onnxruntime_validate.py",
        description="Build ONNX Runtime ROCm wheel (plus minimal ROCm sanity) and print a compact summary.",
    )
    ap.add_argument("--build-dirs", default=None, help="Comma-separated build dirs to validate (default: auto).")
    ap.add_argument("--no-downloads", action="store_true", help="Disable downloads (ORT build step will SKIP).")
    ap.add_argument("--log", action="store_true", help="Write logs under validation/workspace/runs/ (default: off).")
    args = ap.parse_args(argv)

    validate_args: list[str] = ["--profile", "onnxruntime", "--yes", "--no-power"]
    if args.build_dirs:
    ...
    res = find_result(report, name_contains="ONNX Runtime (ROCm)")
    if res is None:
        return rc
    ...
    print("")
    print("==== onnxruntime validation ====")
    print(f"run_dir : {run_dir}")
    print(f"build   : {build_dir}")
    print("profile : onnxruntime")
    ...
    if status != "OK":
        print("")
        print("next steps:")
        print("- Re-run with logs: `python3 validation/scripts/onnxruntime_validate.py --log`")
        print("- Monitor systemd build: `validation/scripts/onnxruntime_rocm/monitor_onnxruntime_rocm_build.sh --once`")
        print("- Check live log: `tail -n 200 validation/workspace/builds/onnxruntime_rocm/build_start.log`")

...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="whisper_validate.py",
        description="Runs the Whisper python validation (GPU required) and prints a compact summary.",
    )
    ...
    validate_args: list[str] = ["--profile", "whisper", "--yes"]
    if args.build_dirs:
    ...
    res = find_result(report, name_contains="Whisper (python)")
    if res is None:
        return rc
    ...
    if status != "OK":
        print("")
        print("notes:")
        print("- This step requires a ROCm-enabled PyTorch + whisper install in the validation venv.")
        print("- If it reports CPU/CUDA torch, GPU validation is treated as FAIL by design.")
        print("")
        print("next steps:")
        print("- Re-run with logs: `python3 validation/scripts/whisper_validate.py --log`")

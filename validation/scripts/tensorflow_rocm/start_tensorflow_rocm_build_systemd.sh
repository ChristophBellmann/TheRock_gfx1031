...
CMD='set -o pipefail; export LIBRARY_PATH="'"${WORK_ROOT}"'/syslibs:${LIBRARY_PATH}"; export LD_LIBRARY_PATH="'"${WORK_ROOT}"'/syslibs:${LD_LIBRARY_PATH}"; "'"${BAZELISK}"'" build --config=opt --config=rocm --verbose_failures //tensorflow/tools/pip_package:wheel 2>&1 | tee -a "'"${LOG_FILE}"'"; exit ${PIPESTATUS[0]}'
